...
if !Vagrant.has_plugin?('vagrant-reload') &&  ENV['SKIP'] != 'true'
  print "Installing vagrant plugins..."
...
Vagrant.configure(2) do |config|
  ...
  config.vm.define 'maxxtuning2018-box' do |node|
    node.vm.hostname = 'maxxtuning2018'
    node.vm.network :private_network, ip: '192.168.40.80'
    node.hostmanager.aliases = %w(maxxtuning2018.test)
  end
